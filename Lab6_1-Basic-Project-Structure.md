...
1. **Docker vs Native Setup**: อธิบายข้อดีของการใช้ Docker เปรียบเทียบกับการติดตั้ง ESP-IDF บน host system
  -/ Docker ใช้งานง่าย ไม่ต้องติดตั้งเยอะ และมี environment เหมือนกันทุกเครื่อง

2. **Build Process**: อธิบายขั้นตอนการ build ของ ESP-IDF ใน Docker container ตั้งแต่ source code จนได้ binary
    1.ใช้คำสั่ง idf.py 
    2.ได้ข้อมูลเป็น binary
    3.สามารถสร้างไฟล์พร้อมกับการ flash

3. ** Make Files**: บทบาทของไฟล์ CMakeLists.txt แต่ละไฟล์คืออะไร และทำงานอย่างไรใน Docker environment?
    -/ ตั้งชื่อโปรเจ็คและโหลด esp-idf กำหนดไฟล์ source ที่จะคอมไพล์

4. **Git Ignore**: ไฟล์ .gitignore มีความสำคัญอย่างไรสำหรับ ESP32 project development?
  -/ ป้องกันไม่ให้ไฟล์ build และไฟล์ขยะถูก commit ขึ้น Git

5. **Container Persistence**: ข้อมูลใดบ้างที่จะหายไปเมื่อ restart container และข้อมูลใดที่จะอยู่ต่อ?
  -/ ไฟล์ใน container ที่ไม่ได้ mount จะหายถ้า restart

6. **Development Workflow**: เปรียบเทียบ workflow การพัฒนาระ
  -/ Docker สะดวก ตั้งค่าเสร็จใช้ได้เลย
