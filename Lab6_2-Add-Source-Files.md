...
1. **Multiple Source Files**: เหตุใดต้องแยก source code เป็นหลายไฟล์?
    -/ เพื่อแยกโค้ดตามหน้าที่ ทำให้แก้ไขง่าย ดูแลสะดวก และลดความซับซ้อนในไฟล์เดียว

2. **CMakeLists.txt Management**: การเพิ่มไฟล์ source ใหม่ต้องแก้ไขอะไรบ้าง?
    -/ ต้องเพิ่มชื่อไฟล์ใหม่ใน SRCS หรือ idf_component_register() เพื่อให้ระบบ build เห็นและ compile

3. **Header Files**: บทบาทของไฟล์ .h คืออะไร และทำไมต้องมี?
    -/ เก็บประกาศฟังก์ชัน struct ให้ไฟล์อื่นเรียกใช้ได้ โดยไม่ต้องรู้รายละเอียดการ implement

4. **Include Directories**: เหตุใด CMakeLists.txt ต้องระบุ INCLUDE_DIRS?
    -/ พื่อบอก compiler ว่าจะไปหาไฟล์ .h จากโฟลเดอร์ไหน

5. **Git Ignore**: ไฟล์ .gitignore ช่วยอะไรในการจัดการ ESP32 project?
    -/ กันไม่ให้ไฟล์ build ชั่วคราวหรือไฟล์ส่วนตัว

6. **Task Management**: การใช้ FreeRTOS task ในโมดูล LED ช่วยอะไร?
    -/ ทำให้ LED ทำงานเป็น background task แยกจาก main logic

7. **Code Organization**: ข้อดีของการแยกโมดูล sensor, display, led เป็นไฟล์แยกคืออะไร?
    -/ ช่วยให้แก้ไขนำกลับมาใช้ซ้ำได้ และลดผลกระทบต่อส่วนอื่นเมื่อแก้โค้ด
...
- จำนวนไฟล์ source: 1
- ขนาด binary: 211456 bytes
- การทำงาน: อ่านและรายงานข้อมูล sensor 

**ขั้นตอนที่ 2 (เพิ่ม display.c):**
- จำนวนไฟล์ source: 2
- ขนาด binary: 220000 bytes
- การทำงาน: นอกจากอ่าน sensor แล้ว มีการแสดงผลข้อมูลผ่าน display module เพิ่มขึ้น

**ขั้นตอนที่ 3 (เพิ่ม led.c):**
- จำนวนไฟล์ source: 3
- ขนาด binary: 230000 bytes
- การทำงาน: นอกจาก sensor และ display ยังเพิ่มการควบคุม LED

**สังเกต:**
- การเพิ่มไฟล์ source ส่งผลต่อขนาด binary อย่างไร?
        -/ จะทำให้ขนาด binary เพิ่มขึ้นตามจำนวนโค้ดและฟีเจอร์ที่เพิ่มเข้ามา

- LED กะพริบทุกกี่วินาที?
    -/ จาก log ดูเหมือน LED จะ toggle ทุกประมาณ 10,000 ms

- แต่ละโมดูลแสดงข้อมูล file และ line อย่างไร?
    -/ จาก log จะเห็นว่าทุกข้อความมีการบอก source file และหมายเลขบรรทัด
